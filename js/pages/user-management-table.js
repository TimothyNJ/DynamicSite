 ...
 *
 * Source: dynamictable55.html prototype.
 * Created: 13-Apr-2026
 *
 * Border toggle button removed — debug borders are controlled by the
 * global show-borders toggle on development/site-settings.
 */
...
function adjustRotatedHeaders() {
  ...
  headers.forEach((header) => {
    // Reset left position to measure original height
    header.style.left = 'calc(var(--umgt-spacing) * 2)';

    const headerHeight = header.offsetHeight;
    const headerWidth = header.offsetWidth;
    const offsetAmount = headerHeight / 2;

    header.style.left = `calc(var(--umgt-spacing) * 2 + ${offsetAmount}px)`;

    // Update pre-rotation box dimensions
    const headerContainer = header.closest('.user-mgmt-header-container');
    const preRotationBox = headerContainer.querySelector('.user-mgmt-pre-rotation-box');
    const dimensionDisplay = preRotationBox.querySelector('.user-mgmt-header-dimensions');

    preRotationBox.style.width = `${headerWidth}px`;
    preRotationBox.style.height = `${headerHeight}px`;
    dimensionDisplay.textContent = `${headerWidth}px × ${headerHeight}px`;
  });
}
...
function updateDimensions() {
  updateElementDimension('user-mgmt-search-dimensions', document.querySelector('.user-mgmt-search-container'));
  updateElementDimension('user-mgmt-center-dimensions', document.querySelector('.user-mgmt-controls-center'));
  updateElementDimension('user-mgmt-toggle-dimensions', document.querySelector('.user-mgmt-toggle-container'));
  updateElementDimension('user-mgmt-controls-right-dimensions', document.querySelector('.user-mgmt-controls-right'));
  updateElementDimension('user-mgmt-table-dimensions', document.querySelector('.user-mgmt-table-wrap'));
}

function updateElementDimension(displayId, element) {
  const display = document.getElementById(displayId);
  if (display && element) {
    display.textContent = `${element.offsetWidth}px × ${element.offsetHeight}px`;
  }
}
 ...
 * Called after the org-roles-above-admin HTML has been injected into the DOM.
 */
