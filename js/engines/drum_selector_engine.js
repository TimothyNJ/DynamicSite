 ...
 * 
 * A specialized version of the 3D component engine for drum/wheel rotation.
 * Based on 3_D_component_engine but constrained to single-axis rotation.
 * 
 * Key differences:
 * - Rotates only around X-axis (horizontal axis through drum)
 * - Preserves all visual features from 3_D_component_engine
 * - Disables sticky rotation for drum mode
 * - Maps all inputs to single-axis rotation
 * 
 * Coordinate System:
...
export class Drum_Selector_Engine {
    constructor(container, config = {}) {
        this.container = typeof container === 'string' ? 
            document.getElementById(container) : container;
            
        if (!this.container) {
            console.error('[Drum Selector Engine] Container not found');
            return;
        }
    ...
    init() {
        if (this.isInitialized) {
            console.warn('[Drum Selector Engine] Already initialized');
            return;
        }
        
        console.log('[Drum Selector Engine] Initializing with config:', this.config);
        ...
        console.log('[Drum Selector Engine] Initialization complete');
    }
    ...
    createTexture() {
        ...
        if (this.config.texture === 'animated') {
            // For cylinders, use multi-material textures to prevent distortion
            if (this.config.geometry === 'cylinder') {
                ...
                console.log('[Drum Selector Engine] Using square texture for geometry:', this.config.geometry);
            }
    ...
    createCylinderTextures() {
        ...
        console.log('[Drum Selector Engine] Cylinder textures created:');
    ...
    createGeometry() {
        ...
        switch (this.config.geometry) {
            ...
            default:
                console.warn(`[Drum Selector Engine] Unknown geometry type: ${this.config.geometry}`);
                geometry = new THREE.BoxGeometry(1, 1, 1);
        }
        
        // Apply subdivision if enabled
        if (this.config.enableSubdivision && this.config.geometry === 'roundedBox') {
            console.log('[Drum Selector Engine] Applying subdivision...');
            geometry = LoopSubdivision.modify(
                geometry,
                this.config.subdivisionIterations,
                this.config.subdivisionParams
            );
            console.log('[Drum Selector Engine] Subdivision complete, vertices:', geometry.attributes.position.count);
        }
    ...
    createMaterial() {
        if (this.config.geometry === 'cylinder' && this.config.texture === 'animated') {
            ...
            console.log('[Drum Selector Engine] Created multi-material array for cylinder');
        } else {
            // Single material for other cases
            const materialConfig = Object.assign({}, this.config.materialParams);
    ...
    setupInteraction() {
        ...
        if ('GestureEvent' in window) {
            console.log('[Drum Selector Engine] GestureEvent is supported - adding listeners');
            ...
            console.log('[Drum Selector Engine] GestureEvent is NOT supported in this browser');
        }
    ...
    onGestureStart(event) {
        ...
        console.log('[Drum Selector Engine] Gesture start - rotation:', this.gestureRotation, 'scale:', this.gestureScale);
    }
    ...
    onGestureChange(event) {
        ...
        if (event.rotation !== undefined) {
            ...
            console.log('[Drum Selector Engine] Gesture change - rotation:', event.rotation, 'delta (radians):', rotationDelta);
            ...
            if (Math.abs(rotationDelta) > rotationThreshold) {
                ...
                // Reset auto-rotation time
                this.autoRotationTime = 0;
                console.log('[Drum Selector Engine] Rotation applied!');
    ...
    updateAnimatedTexture(time) {
        if (this.config.geometry === 'cylinder' && this.sideContext && this.capContext) {
            // Update multi-material textures for cylinder
    ...
    setRotationSpeed(speed) {
        ...
        console.log('[Drum Selector Engine] Rotation speed set to:', speed);
    }
    ...
    setLightPosition(lightName, axis, value) {
        if (this.lights[lightName]) {
            ...
            console.log(`[Drum Selector Engine] ${lightName} light ${axis} set to:`, value);
        }
    }
    
    // Cleanup method
    dispose() {
        console.log('[Drum Selector Engine] Disposing...');
