...
export class drum_wheel_3d_component_engine {
    ...
    createTexture() {
        ...
        if (this.config.texture === 'animated') {
            ...
            if (this.config.geometry === 'cylinder') {
                ...
                console.log('[Drum Wheel Engine] Cylinder texture dimensions:', textureWidth, 'x', textureHeight);
            } else {
    ...
    createGeometry() {
        ...
        switch (this.config.geometry) {
            ...
            default:
                console.warn(`[Drum Wheel 3D Engine] Unknown geometry type: ${this.config.geometry}`);
                geometry = new THREE.BoxGeometry(1, 1, 1);
        }
        
        // Apply subdivision if enabled
        if (this.config.enableSubdivision && this.config.geometry === 'roundedBox') {
            console.log('[Drum Wheel 3D Engine] Applying subdivision...');
            geometry = LoopSubdivision.modify(
                geometry,
                this.config.subdivisionIterations,
                this.config.subdivisionParams
            );
            console.log('[Drum Wheel 3D Engine] Subdivision complete, vertices:', geometry.attributes.position.count);
        }
    ...
    setupInteraction() {
        ...
        if ('GestureEvent' in window) {
            console.log('[Drum Wheel 3D Engine] GestureEvent is supported - adding listeners');
            ...
            console.log('[Drum Wheel 3D Engine] GestureEvent is NOT supported in this browser');
        }
    ...
    onGestureStart(event) {
        ...
        console.log('[Drum Wheel 3D Engine] Gesture start - rotation:', this.gestureRotation, 'scale:', this.gestureScale);
    }
    ...
    onGestureChange(event) {
        ...
        if (event.rotation !== undefined) {
            ...
            console.log('[Drum Wheel 3D Engine] Gesture change - rotation:', event.rotation, 'delta (radians):', rotationDelta);
            ...
            if (Math.abs(rotationDelta) > rotationThreshold) {
                ...
                // Reset auto-rotation time
                this.autoRotationTime = 0;
                console.log('[Drum Wheel 3D Engine] Rotation applied!');
    ...
    setLightPosition(lightName, axis, value) {
        if (this.lights[lightName]) {
            ...
            console.log(`[Drum Wheel 3D Engine] ${lightName} light ${axis} set to:`, value);
        }
    }
    
    // Cleanup method
    dispose() {
        console.log('[Drum Wheel 3D Engine] Disposing...');
